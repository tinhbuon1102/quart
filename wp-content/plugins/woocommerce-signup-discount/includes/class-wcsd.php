  ...
  */
  public function __construct() {
    ...
    if( get_option( 'wcsd_popup' ) == 'yes'  ) {
      ...
      $cookie =  ( isset( $_COOKIE['wcsd_closed'] ) && $_COOKIE['wcsd_closed'] == 'yes' ) ? 'yes' : 'no';

      if( get_option( 'wcsd_popup' ) == 'yes' && $cookie == 'no'  ) {
  ...
  public function wcsd_enque_admin_scripts() {
    if( is_admin() && isset($_GET['tab']) && $_GET['tab'] == 'signup_discount' ) {
  ...
  */
  public function wcsd_enque_scripts() {
    ...
    if( !is_user_logged_in() && get_option( 'wcsd_enabled' ) == 'yes' ) {
      ...
      wp_enqueue_script( 'wcsd-custombox-script', plugins_url( 'assets/js/jquery.magnific-popup.min.js', WCSD_FILE ) , array( 'jquery' ), '1.0.0', true);
      wp_enqueue_script('custom-script-signup-discount', plugins_url( 'assets/js/custom.js', WCSD_FILE ), array( 'jquery', 'wcsd-custombox-script' ), '1.0.0', true );

      //Add custombox css 
      wp_enqueue_style( 'wcsd-custombox-stylesheet', plugins_url( 'assets/css/magnific-popup.css', WCSD_FILE ));     

      $overlay_color = get_option( 'wcsd_pop_overlay' );
      list($r, $g, $b) = sscanf($overlay_color, "#%02x%02x%02x");
      $rgb_color = 'rgba('.$r.','.$g.','.$b.','.get_option( 'wcsd_overlay_opacity' ).')';

      wp_localize_script('custom-script-signup-discount', 'wcsd', 
        array(
          'effect'          => get_option('wcsd_popup_animation'),
          'overlayColor'    => $rgb_color,
          'delay'           => get_option( 'wcsd_dis_seconds'),
          'cookie_length'   => get_option( 'wcsd_cookie_length' ),
          'exit_intent'     => get_option( 'wcsd_exit_intent' ),
          'hinge'           => get_option( 'wcsd_hinge' ),
          'overlay_click'   => get_option( 'wcsd_overlay_click' ),
        )
      );  
    }
  }
    ...
    public function wcsd_send_coupons( $user_id ) {
      ...
      if( $mail != 'yes' ){
        ...
        if ( version_compare( $woocommerce->version, '2.3',  ">=" ) ) {
          $mailer = WC()->mailer();
          $mailer->send( $user->user_email, $subject, $mailer->wrap_message( $subject, $body ), $headers, '' );
        }
        else
