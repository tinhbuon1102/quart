...
class WC_Settings_Signup_Discount extends WC_Settings_Page {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_admin_settings_sanitize_option_wcsd_email', array( $this, 'wcsd_save_editor_val' ), 10, 3 );
		add_action( 'woocommerce_admin_settings_sanitize_option_wcsd_popup_text', array( $this, 'wcsd_save_editor_val' ), 10, 3 );
	...
	public function get_settings() {
		...
		if ( $myaccount_page_id ) {
  		$myaccount_page_url = get_permalink( $myaccount_page_id );
			...
			$myaccount_page_url = get_site_url().'/my-account';
		}
        ...
        array(
					'title' 		=> __( 'Exit intent', 'wc_signup_discount' ),
					'desc' 			=> __( 'Display popup based on exit intent', 'wc_signup_discount' ),
					'type' 			=> 'checkbox',
					'id'				=> 'wcsd_exit_intent',
					'default' 	=> 'no'										
				),
        array(
					'title' 		=> __( 'Close popup on overlay click', 'wc_signup_discount' ),
					'desc' 			=> __( 'Close the popup when people click outside the popup?', 'wc_signup_discount' ),
					'type' 			=> 'checkbox',
					'id'				=> 'wcsd_overlay_click',
					'default' 	=> 'no'										
				),
				array(
					'title' 	  => __( 'Popup cookie length(days)', 'wcmd' ),
					'id' 		  	=> 'wcsd_cookie_length',
					'type' 		  => 'number',
					'desc' 		  => __( 'Enter the value for number of days the site should remember the visitor.', 'wc_signup_discount' ),
					'default'	  => '30',
					'css' 		  => 'width: 60px;',
					'desc_tip'	=>  true
				),																
				...
				array(
					'title' 	  => __( 'Popup close button Color', 'wc_signup_discount' ),
					'id' 		  	=> 'wcsd_close_color',
					'type' 		  => 'color',
					'default'	  => '#fff',
					'css' 		  => 'width: 125px;',
				...
				array(
					'title' 	  => __( 'Popup Overlay Color', 'wc_signup_discount' ),
					'id' 		  	=> 'wcsd_pop_overlay',
					'type' 		  => 'color',
					'default'	  => '#2e3865',
					'css' 		  => 'width: 125px;',
        ...
        array(
					'title'   	=> __( 'Popup overlay opacity', 'wc_signup_discount' ),
					'desc' 	  	=> __( 'Enter a value for the opacity value of the popup background.', 'wc_signup_discount' ),
					'type' 	  	=> 'number',
					'id'	  		=> 'wcsd_overlay_opacity',
					'css' 	  	=> 'width: 125px;',
					'default' 	=> '0.8',
				...
				array(
					...
					'options' 	=> array(
						'mfp-with-fade' 		=> __( 'Fade In', 'wc_signup_discount' ),
						'mfp-tada'					=> __( 'Tada', 'wc_signup_discount' ),
						'mfp-shake' 				=> __( 'Shake', 'wc_signup_discount' ),
						'mfp-zoom-out' 			=> __( 'Zoom Out', 'wc_signup_discount' ),
						'mfp-zoom-in' 			=> __( 'Zoom In', 'wc_signup_discount' ),
						'mfp-3d-unfold' 		=> __( '3D Unfold', 'wc_signup_discount' ),
						'mfp-3d-sign' 			=> __( '3D Sign', 'wc_signup_discount' ),
						'mfp-move-from-top' => __( 'Move From Top', 'wc_signup_discount' ),
						'mfp-move-horizontal' => __( 'Move Horizontal', 'wc_signup_discount' ),
						'mfp-slide-right' 		=> __( 'Slide Right', 'wc_signup_discount' ),
						'mfp-newspaper' 			=> __( 'Newspaper', 'wc_signup_discount' ),
          			),
					'default' 							=> 'mfp-with-fade'
				),
        array(
					'title' 			=> __( 'Close hinge effect', 'wc_signup_discount' ),
					'desc' 				=> __( 'Enable hinge effect when closing the modal.', 'wc_signup_discount' ),
					'type' 				=> 'checkbox',
					'id'					=> 'wcsd_hinge',
					'default' 		=> 'no'											
				),
				array(
					'title' 	  	=> __( 'Display popup after(seconds)', 'wc_signup_discount' ),
					'id' 		  		=> 'wcsd_dis_seconds',
					'type' 		  	=> 'number',
					'css' 		  	=> 'width: 45px;',
					'default'	  	=> '3',
				...
				array(
					...
					'default'	  	=> '<h1 style="text-align: center;"><span style="color: #ffffff;">Get 5% Off now!</span></h1>
													...
													<h5 style="text-align: center;"><span style="color: #8B1C1A;">&nbsp;<a id="wcsd_set_cook" style="color: #8B1C1A;" href="'.$myaccount_page_url.'">Register Now</a></span></h5>',
					'desc_tip'	  =>  true
				),				
	...
	*
	*/
	public function wcsd_save_editor_val( $value, $option, $raw_value ) {
		...
		<script type="text/javascript">
			jQuery(function($){
				...
				// Ajax product search box
					$( ':input.wcsd.wc-product-search' ).each( function() {
					var select2_args = {
						...
						ajax: {
							...
							data: function( term ) {
								return {
									term:     term,
									action:   'wcsd_ajax_products',
									security: '<?php echo wp_create_nonce( "wcsd-search-products" ); ?>',
					...
					//select2_args = $.extend( select2_args, wcsd_getEnhancedSelectFormatString() );
