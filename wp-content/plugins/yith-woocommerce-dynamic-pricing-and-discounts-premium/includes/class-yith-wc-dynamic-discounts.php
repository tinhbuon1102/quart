	...
	class YITH_WC_Dynamic_Discounts {
                    ...
                    foreach ( $cart_rule['rules'] as $index => $r ) {
                        ...
                        switch ( $discount_type ) {
	                        ...
	                        case 'brand_list_excluded':
		                        if( isset( $r['rules_type_' . $discount_type] )  && defined( 'ICL_SITEPRESS_VERSION' ) && YITH_WC_Dynamic_Pricing()->get_option( 'wpml_extend_to_translated_object' ) == 'yes' ) {
		...
		public function apply_discount() {
			...
			if ( ! empty ( $rules ) && $discount > 0 ) {
				...
				if( version_compare( WC()->version, '2.7', '>' ) &&  $coupon = $this->get_current_coupon() ){
					$coupon->delete();
				}
			}
		}
