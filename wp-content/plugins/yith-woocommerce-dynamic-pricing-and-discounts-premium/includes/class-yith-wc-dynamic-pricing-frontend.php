...
if ( ! class_exists( 'YITH_WC_Dynamic_Pricing_Frontend' ) ) {
	...
	class YITH_WC_Dynamic_Pricing_Frontend {
		...
		function get_price_html( $price, $product ) {
			...
			$display_regular_price = function_exists('wc_get_price_to_display') ? wc_get_price_to_display( $product, array( 'qty' => 1, 'price' => $product->get_regular_price()  ) ): $product->get_display_price( $product->get_regular_price() );
		...
		public function get_minimum_price( $product, $min_quantity = '' ) {
			...
			if( $min_quantity != '' && ! $min_quantity_check ){
