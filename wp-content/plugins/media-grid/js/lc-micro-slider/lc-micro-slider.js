...
(function ($) {
	var lc_micro_slider = function(element, lcms_settings) {
		...
		var slider_setup = function($wrap_obj) {
			...
			$wrap_obj.html('<div class="lcms_wrap"></div>');
			...
			if(settings.nav_arrows && vars.slides.length > 1) {
				$wrap_obj.find('.lcms_wrap').append('<div class="lcms_nav"><span class="lcms_prev"></span><span class="lcms_next"></span></div>');	
			}
		...
		var populate_slide = function($wrap_obj, type, slide_index) {
			...
			var slide_code = '<div class="lcms_slide '+ fx_class +' '+ preload_class +'" rel="'+ slide_index +'"><div class="lcms_inner">'+ bg + contents +'</div>'+ loader_code +'</div>';
			
			// populate
			$wrap_obj.find('.lcms_wrap').append(slide_code);	
		...
		lcms_slide = function($wrap_obj, direction) {
			...
			// trigger custom action | args: new slide index - new slide object - nav direction
			$wrap_obj.trigger('lcms_changing_slide', [new_index, vars.slides[new_index], direction]);
