 ...
 * Plugin URI: https://www.dreamfoxmedia.com/portfolio/woocommerce-payment-gateway-per-product-premium/
 * Version: 1.2.8
...
if (is_plugin_active('woocommerce-product-payments-premium/woocommerce-product-payments-premium.php')) {
    add_action('admin_notices', 'softsdev_wpp_activation_notice');
    function softsdev_wpp_activation_notice() {
        ?>
        <div class="error notice is-dismissible">
            <p><strong>Woocommerce Payment Gateway Per Product Premium</strong> plugin is activate so free plugin will not work.</p>
        </div>
        <?php
    }
...
if (!is_plugin_active('woocommerce-product-payments-premium/woocommerce-product-payments-premium.php') && in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
    ...
    function softsdev_product_payments_settings() {
        ...
        <div class="left-mc-setting">
            ...
            <p>This version is limited in features. For a really small fee of 19,95 you can get the Premium version with <a href="https://www.dreamfoxmedia.com/shop/plugins/woocommerce-payment-gateway-per-product-premium#utm_source=plugin&utm_medium=premium_left&utm_campaign=wcpgpp-f">More Settings</a>!</p>
        ...
        <div class="right-mc-setting">
            <div style="border: 5px dashed #B0E0E6; padding: 0 20px; background: white;">
                ...
                <p>This plugin has a Premium version with More Setting. <a href="https://www.dreamfoxmedia.com/shop/plugins/woocommerce-payment-gateway-per-product-premium#utm_source=plugin&utm_medium=premium_right&utm_campaign=wcpgpp-f">Have a look at its benefits</a>!</p>
            </div>
    ...
    function softsdev_get_category_ul($categories, $selected_cats, $class, $parent_id, $id = '', $name = '') {
        ...
        function is_product_eligible($product_id) {
            ...
            if ($softsdev_selected_cats) {
                $sql = "SELECT count(*) as product_count FROM $wpdb->posts LEFT JOIN $wpdb->term_relationships ON ($wpdb->posts.ID = $wpdb->term_relationships.object_id) INNER JOIN $wpdb->postmeta ON ( $wpdb->posts.ID = $wpdb->postmeta.post_id ) WHERE 1=1 AND ( $wpdb->term_relationships.term_taxonomy_id IN ($softsdev_selected_cats) ) AND ( ( $wpdb->postmeta.meta_key = '_visibility' AND $wpdb->postmeta.meta_value IN ('catalog','visible') ) ) AND $wpdb->posts.post_type = 'product' AND (($wpdb->posts.post_status = 'publish')) AND ID = $product_id GROUP BY $wpdb->posts.ID ORDER BY $wpdb->posts.post_date DESC";
        ...
        <?php softsdev_get_category_ul($cats, $softsdev_selected_cats, 'categorychecklist form-no-clear', 0, 'product_catchecklist'); ?>
                    </select>
                </div>
