...
class PluginOrganizer {
	...
	function display_admin_debug() {
		if (get_option('PO_display_debug_msg') == 1) {
			...
			if (array_diff($debugRoles, $roles) !== $debugRoles && get_class($PluginOrganizerMU) == 'PluginOrganizerMU' && sizeof($PluginOrganizerMU->debugMsg) > 0) {
				?>
				<div class="notice notice-warning" id="po_debug_msg_container" style="<? print (isset($POAdminStyles['admin_debug_style']))? $POAdminStyles['admin_debug_style'] : 'padding: 20px;'; ?>">
	...
	function display_footer_debug() {
		if (get_option('PO_display_debug_msg') == 1) {
			...
			if (array_diff($debugRoles, $roles) !== $debugRoles && get_class($PluginOrganizerMU) == 'PluginOrganizerMU' && sizeof($PluginOrganizerMU->debugMsg) > 0) {
				...
				?>
				<div id="po_debug_msg_container" style="<? print (isset($POAdminStyles['front_debug_style']))? $POAdminStyles['front_debug_style'] : 'position: relative;z-index: 99999;background: #fff;width: 100%;border: 4px solid #000;padding: 10px;'; ?>">
					<div style="font-weight: bold;margin-bottom:10px;">Plugin Organizer Debug Messages</div>
					<hr>
					<?php
					foreach($PluginOrganizerMU->debugMsg as $debugMsg) {
	...
	function check_version() {
		...
		if (get_option("PO_version_num") != "9.5.1" && !in_array($pagenow, array("plugins.php", "update-core.php", "update.php"))) {
			$this->activate();
		}
	}
	...
	function activate() {
		...
		if (get_option("PO_version_num") != "9.5.1") {
			update_option("PO_version_num", "9.5.1");
		}
	...
	function compatibility_notices() {
		...
		if (count($noticesArray) > 0) {
			...
			<div class="notice notice-error" id="PO_compat_notices" style="padding: 20px;">
				...
				?>
				<a href="#" id="PO_disable_compat_notices">Disable this warning</a>
			</div>
			<script type="text/javascript" language="javascript">
