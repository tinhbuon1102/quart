...
function add_footer_menu_shop() {
    $shopmenu = get_terms('product_cat', array('hide_empty' => 1, 'orderby' => 'id', 'order' => 'DESC', 'parent' => 0)); //, 'exclude' => '17,77'		
    ?>
    <div class="primary">
...
function elsey_woocommerce_payment_gateways($load_gateways) {
    ...
    $hide_cod = true;
    $arr_cat_except_to_hide = array('hatcap', 'clothing', 'jacket', 'ourter', 'coat', 'tops', 'tshirt', 'sweatshirts', 'shirt', 'sweat', 'hoodie', 'bottoms', 'jewelry', 'bracelets', 'earrings', 'rings');
...
//End
//Auto deactive Woocommerce Signup Discount plugin from Dec 17th 23:59
function deactive_woocommerce_signup_discount() {
    $schedule_specific = '2019-12-10 21:00:00';
    $schedule_all = '2019-12-17 23:59:00';
    if (current_time('timestamp') >= strtotime($schedule_specific) && current_time('timestamp') < strtotime($schedule_all)) {
        //keep active plugin
    } else {
        if (is_plugin_active('woocommerce-signup-discount/woocommerce-signup-discount.php')) {
            deactivate_plugins('woocommerce-signup-discount/woocommerce-signup-discount.php');
        }
    }
}

add_action('admin_init', 'deactive_woocommerce_signup_discount');
